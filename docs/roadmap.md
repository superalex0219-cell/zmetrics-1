...
*Реализовано 2026-06-11 (сессия rev 14); рендер на бэкенде (а не в worker — формат
запрашивается лениво), визуалы — в пайплайне.*

- [x] Worker: шаг `report_visuals` — оверлей масок на исходном кадре + колоризованная
      карта глубины (PNG в MinIO под префиксом джоба); вспомогательный, никогда не
      валит джоб (деградация в metadata)
- [x] Backend: `app/services/report_export.py` — сборка полного бандла (результат,
      паспорт БВР, карьер/взрыв, устройство, рекомендации) + рендеры: PDF
      (WeasyPrint+jinja2, DejaVu для кириллицы), DOCX (python-docx), XLSX (openpyxl,
      4 листа), CSV (utf-8-sig), JSON
- [x] `GET /reports/{id}/export?format=pdf|docx|xlsx|csv|json`; файл также
      сохраняется в MinIO `reports/{id}/` (часть STORE-1)
- [x] SAFETY: метод анализа — бейдж в каждом формате (mock → «⚠ Синтетические
      данные», красным); рекомендации всегда со статусом, параметры — «справочно,
      не применяются автоматически»
- [x] Desktop: выбор формата + «Экспорт…» на экране отчёта (blaster+)
- [ ] Прогон на реальном отчёте с визуалами (нужен job после пересборки worker)
